...
export const useUsuarioCursoStore = defineStore("usuarioCurso", () => {
  ...
  async function fetchAllInscripciones() {
    try {
      const response = await fetch("http://34.198.50.70:3000/api/UsuarioCurso");
  ...
  async function fetchInscripcionesByUsuarioId(idUsuario: number) {
    try {
      const response = await fetch(`http://34.198.50.70:3000/api/UsuarioCurso/usuario/${idUsuario}`);
  ...
  async function fetchInscripcionesByCursoId(idCurso: number) {
    try {
      const response = await fetch(`http://34.198.50.70:3000/api/UsuarioCurso/curso/${idCurso}`);
  ...
  async function createInscripcion(nuevaInscripcion: UsuarioCursoDTO) {
    try {
      console.log("Enviando POST a http://34.198.50.70:3000/api/UsuarioCurso:", nuevaInscripcion);
  ...
  async function deleteInscripcion(idUsuario: number, idCurso: number) {
    try {
      ...
      const response1 = await fetch(`http://34.198.50.70:3000/api/UsuarioCurso/${idUsuario}/${idCurso}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" }
      });
      ...
      const response2 = await fetch(`http://34.198.50.70:3000/api/UsuarioCurso?idUsuario=${idUsuario}&idCurso=${idCurso}`, {
        method: "DELETE",
        headers: { "Content-Type": "application/json" }
      });
      ...
      const response3 = await fetch("http://34.198.50.70:3000/api/UsuarioCurso/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          idUsuario: idUsuario,
      ...
      if (inscripcionEspecifica) {
        ...
        if (posibleId) {
          ...
          const response4 = await fetch(`http://34.198.50.70:3000/api/UsuarioCurso/${posibleId}`, {
            method: "DELETE"
          });
