...
export const useQuizStore = defineStore("quiz", () => {
  ...
  async function fetchAllQuizzes(): Promise<QuizCompletaDTO[]> {
    ...
    try {
      ...
      const response = await fetch("http://34.198.50.70:5000/api/quiz");
  ...
  async function fetchQuizById(idQuiz: number): Promise<QuizDTO | null> {
    try {
      ...
      const url = `http://34.198.50.70:5000/api/quiz/${idQuiz}`;
  ...
  async function fetchQuizzesByUsuario(idUsuario: number): Promise<QuizCompletaDTO[]> {
    try {
      ...
      const response = await fetch(`http://34.198.50.70:5000/api/quiz/usuario/${idUsuario}`);
  ...
  async function fetchQuizzesByCurso(idCurso: number): Promise<QuizCompletaDTO[]> {
    ...
    try {
      ...
      const response = await fetch(`http://34.198.50.70:5000/api/quiz/curso/${idCurso}`);
  ...
  async function updateQuiz(idQuiz: number, updatedData: Partial<QuizDTO>): Promise<QuizDTO | null> {
    try {
      ...
      const response = await fetch(`http://34.198.50.70:5000/api/quiz/${idQuiz}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json"
        },
  ...
  async function deleteQuiz(idQuiz: number, idUsuario?: number): Promise<boolean> {
    ...
    try {
      ...
      let url = `http://34.198.50.70:5000/api/Quiz/${idQuiz}`;
      if (idUsuario) {
      ...
      try {
        ...
        const fallbackUrl = `http://34.198.50.70:5000/api/quiz/${idQuiz}${idUsuario ? `?idUsuario=${idUsuario}` : ''}`;
