...
export const usePeticionProfesorStore = defineStore("peticionProfesor", () => {
  ...
  const createPeticion = async (data: PeticionProfesorData): Promise<boolean> => {
    ...
    try {
      ...
      const response = await fetch("https://academiqapi.retocsv.es/api/PeticionProfesor", {
        method: 'POST',
        headers: {
  ...
  async function rechazarPeticion(idPeticion: number): Promise<boolean> {
    try {
      const response = await fetch(`https://academiqapi.retocsv.es/api/PeticionProfesor/rechazar/${idPeticion}`, {
        method: "DELETE"
      });
