...
export const useFavoritoStore = defineStore('favorito', {
  ...
  actions: {
    ...
    async cargarFavoritos() {
      ...
      this.loading = true
      try {
        const res = await axios.get(`https://academiqapi.retocsv.es/api/favorito/usuario/${usuarioLogeadoStore.usuarioActual.idUsuario}`, {
          headers: {
    ...
    async toggleFavorito(idVideo: number | string) {
      ...
      try {
        const res = await axios.post(
          `https://academiqapi.retocsv.es/api/favorito/toggle/${idVideoNum}`, 
          {}, 
          {
            headers: {
              'Authorization': `Bearer ${usuarioLogeadoStore.token}`,
        ...
        if (añadido) {
          if (!this.esFavorito(idVideoNum)) {
            try {
              const videoRes = await axios.get(`https://academiqapi.retocsv.es/api/video/${idVideoNum}`, {
                headers: {
                  'Authorization': `Bearer ${usuarioLogeadoStore.token}`
                }
              })
