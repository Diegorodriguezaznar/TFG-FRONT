...
export const useFavoritoStore = defineStore('favorito', {
  ...
  actions: {
    ...
    async cargarFavoritos() {
      ...
      this.loading = true
      try {
        const res = await axios.get(`http://34.198.50.70:3000/api/favorito/usuario/${usuarioLogeadoStore.usuarioActual.idUsuario}`, {
          headers: {
    ...
    async toggleFavorito(idVideo: number | string) {
      ...
      try {
        const res = await axios.post(
          `http://34.198.50.70:3000/api/favorito/toggle/${idVideoNum}`, 
          {}, 
          {
            headers: {
              'Authorization': `Bearer ${usuarioLogeadoStore.token}`,
        ...
        if (añadido) {
          if (!this.esFavorito(idVideoNum)) {
            try {
              const videoRes = await axios.get(`http://34.198.50.70:3000/api/video/${idVideoNum}`, {
                headers: {
                  'Authorization': `Bearer ${usuarioLogeadoStore.token}`
                }
              })
