...
export const useUsuarioStore = defineStore("usuario", () => {
  ...
  async function fetchUsuarioById(idUsuario: number) {
    try {
      const response = await fetch(`http://localhost:5000/api/Usuario/${idUsuario}`);
  ...
  async function updateUsuario(updatedUser: UsuarioDTO) {
    try {
      const response = await fetch(`http://localhost:5000/api/Usuario/${updatedUser.idUsuario}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function aceptarUsuarioComoProfesor(idUsuario: number): Promise<boolean> {
    try {
      const response = await fetch(`http://localhost:5000/api/Usuario/${idUsuario}/aceptar-profesor`, {
        method: "PUT",
        headers: {
