...
export const useReporteStore = defineStore('reporte', () => {
  ...
  async function crearReporte(nuevoReporte: ReporteDTO) {
    ...
    try {
      ...
      const response = await fetch('http://34.198.50.70:5000/api/ReporteVideo', {
        method: 'POST',
        headers: {
  ...
  async function fetchReportesPorVideo(idVideo: number) {
    ...
    try {
      const response = await fetch(`http://34.198.50.70:5000/api/ReporteVideo/video/${idVideo}`)
  ...
  async function verificarReporteExistente(idUsuario: number, idVideo: number) {
    ...
    try {
      const response = await fetch(`http://34.198.50.70:5000/api/ReporteVideo/check?idUsuario=${idUsuario}&idVideo=${idVideo}`)
