...
<template>
  <v-dialog :model-value="props.mostrar" max-width="400px" @update:model-value="emit('update:mostrar', $event)">
    <v-card>
      <v-card-title class="text-h6">Confirmar aprobación</v-card-title>
      <v-card-text>
        ¿Estás seguro de que quieres aprobar el video
        <strong>{{ video?.titulo }}</strong>?
      </v-card-text>
      <v-card-actions>
        <v-spacer />
        <v-btn text @click="emit('update:mostrar', false)">Cancelar</v-btn>
        <v-btn color="success" @click="emit('confirmar', video?.idVideo)">Aprobar</v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
</template>